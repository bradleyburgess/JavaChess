...
public class CoordinateValidator {
    public static void validateRankFile(char file, int rank) throws InvalidCoordinateException {
        if (file < 'a' || file > 'h') throw new InvalidCoordinateException("File must be between 'a' to 'h'");
        if (rank < 1 || rank > 8) throw new InvalidCoordinateException("Rank must be between 1 and 8");
    }
